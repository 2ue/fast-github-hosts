...
def generate_hosts_file(
    ...
    hosts_content.append(f"# 原始文件: https://raw.githubusercontent.com/2ue/fast-github-hosts/main/github_hosts_pro")
    hosts_content.append("#")
    hosts_content.append("# 统计信息:")
    hosts_content.append(f"#   - 域名级别: {level.upper()} ({'20个核心' if level == 'core' else '63个扩展' if level == 'extended' else '141个完整'})")
    hosts_content.append(f"#   - 成功解析: {success_count}/{len(domains)} ({success_rate})")
    hosts_content.append(f"#   - 多IP轮询: {'是 (每域名' + str(TOP_IP_COUNT) + '个最快IP)' if multi_ip else '否 (仅最快IP)'}")
    hosts_content.append("#")
    hosts_content.append("# 技术方案:")
    hosts_content.append(f"#   - DNS查询: {'DoH (防污染)' if use_doh else '传统DNS'} - 多源并发查询")
    hosts_content.append(f"#   - 测速方法: 纯TCP连接测试 (端口{TCP_PORT})")
    hosts_content.append(f"#   - 测速次数: {TCP_TEST_COUNT}次取中位数 (超时{TCP_TIMEOUT}秒)")
    hosts_content.append(f"#   - 并发处理: {MAX_WORKERS}个worker并发")
    hosts_content.append(f"#   - 智能缓存: {'已启用' if use_cache else '已禁用'}")
    hosts_content.append("#")
    hosts_content.append("# 使用说明:")
    ...
    hosts_content.append("#   手动安装:    复制下方内容追加到系统hosts文件")
    hosts_content.append("#")
    hosts_content.append("# 更新建议:")
    hosts_content.append("#   - 自动更新: 配置cron/Task Scheduler每天执行")
    hosts_content.append("#   - 手动更新: 每周执行一次以获取最新IP")
    hosts_content.append("#")
